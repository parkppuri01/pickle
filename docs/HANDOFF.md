...
> **마지막 업데이트**: 2026-06-14 · **🎉 1.1.0 릴리스(MARKETING_VERSION=1.1.0, build 7)** · **현재 상태**: 1.1 출시 — **펜 색상 대폭 강화**: 무지개 색상환 버튼으로 여는 **인라인 색상 피커**(채도/명도 사각형 + 색조 바, 팝오버 내부 — 시스템 색상창 안 씀)·**스포이드**(`NSColorSampler` 화면 색 추출)·**색상 hex(#RRGGBB) 클릭 복사**. 옵션 팝오버는 작은 창에서도 안 잘리게 `ScrollView`+높이 캡. **Sparkle 자동 업데이트 호스팅**(pizzaClip web → `pizza-clip.com/pickle/appcast.xml`)로 1.1.0 DMG·appcast 배포 완료. ↓아래 [✅ 2026-06-14 — 1.1.0] 참고.
> **새 세션은 이 문서를 먼저 읽으면 컨텍스트가 잡힙니다.**
> **▶ 다음 세션 할 일: 1.1 출시 후 새 요청부터. 다음 릴리스(1.2+)는 [다음 릴리스(1.1+) 재현 순서] 그대로(버전만 ↑). 알려진 후속(보류): 편집 입력칸이 캔버스 `scaleEffect` 안에 있어 한글 IME 후보창 위치가 살짝 어긋날 수 있음(입력칸만 scaleEffect 밖으로 분리 권고).**
